...
def load_date_ranges(path="coin_data_ranges.txt"):  # ✅ changed from ticker_date_ranges.txt
    ranges = {}
...
def backtest(ticker, start_date, end_date, starting_balance):
    path = os.path.join("crypto_data", f"{ticker}.csv")  # ✅ changed from stock_data
...
def main():
    ...
    ticker = input("Enter coin: ").strip().upper()

    if ticker not in date_ranges:
        print(f"{ticker} not found in coin_data_ranges.txt")  # ✅ updated name in message
        return
